package Ejercicio01.relacion12.pizzeria;

import java.util.LinkedList;
import java.util.Queue;

public class ColaPedidos {
	private LinkedList<Pedido> listaDePedidos;

	public ColaPedidos() {
		listaDePedidos = new LinkedList<Pedido>();
	}

	public void agregarPedido(Pedido p) {
		listaDePedidos.add(p);
	}

	public Pedido atenderPedido() throws PizzaException {

		if (listaDePedidos.isEmpty()) {
			throw new PizzaException("No hay pedidos en la cola");
		}
		return listaDePedidos.poll();
	}

	public int numeroDePedidosEnCola() {
		
		return listaDePedidos.size();
	}
	public Pedido mostrarSiguientePedido() throws PizzaException {
		if (listaDePedidos.isEmpty()) {
			throw new PizzaException("No hay pedidos en la cola");
		}
		return listaDePedidos.peek();
	}
}
